...
show_fraction = 0.00; % 1: show everything, 0: show nothing, 0.1: show every tenth, and so on

% Stuff related to learning agent (YOU SHOULD EXPERIMENT A LOT WITH THESE
% SETTINGS - SEE EXERCISE 6).
rewards = struct('default', 0, 'apple', 1, 'death', -1.5); % Reward signal
gamm    = 0.9;  % Discount factor in Q-learning
alph    = 0.2; % Learning rate in Q-learning (automatically set to zero during testing)
eps     = 0.005; % Random action selection probability in epsilon-greedy Q-learning (automatically set to zero during testing)
