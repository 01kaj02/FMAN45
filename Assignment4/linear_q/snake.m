...
rewards   = struct('default', 0, 'apple', 30, 'death', -200); % Experiment with different reward signals, to see which yield a good behaviour for the agent
gamm      = 0.985;                                          % Discount factor in Q-learning
alph      = 0.445;                                           % Learning rate in Q-learning
eps       = 0.06;                                           % Random action selection probability in epsilon-greedy Q-learning (lower: increase exploitation, higher: increase exploration)
...
alph_update_factor = 0.705; % At alpha update: new alpha = old alpha * alph_update_factor
eps_update_iter    = 1000;  % 0: Never update eps, Positive integer k: Update eps every kth episode
eps_update_factor  = 0.605; % At eps update: new eps = old eps * eps_update_factor
