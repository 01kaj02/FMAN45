...
for action = 1 : 3 % Evaluate all the different actions (left, forward, right).
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if grid(row, col) > 0
        ...
        state_action_feats(1, action) = 1;
    end  % does the snake die next move 
    

    [apple_row, apple_col] = find(grid == -1);
     if isempty(apple_row) || isempty(apple_col)
        state_action_feats(2, action) = 0;
     else 
        prevdist = norm(prev_head_loc-[apple_row, apple_col]);
        distance = norm(next_head_loc - [apple_row, apple_col]);
        state_action_feats(2, action) = sign(prevdist-distance);  % does the snake get closer to the apple 
    end 
    ...
    if isempty(apple_row) || isempty(apple_col)
        ...
        applevector = [apple_row, apple_col] - head_loc;
        dirvector = next_head_loc - head_loc; 
        state_action_feats(3, action) = sign(dot(applevector, dirvector)); % is it moving in the right direction 
    end 
...
%                 rr = row + dr;
%                 cc = col + dc;
%                 if rr > 1 && rr < N && cc > 1 && cc < N && grid(rr, cc) <= 0
%                     freespots = freespots + 1;
%                 end
%             end
%         end
%     end
    %state_action_feats(4, action) = (freespots / 4) * 2 - 1; % how many free spaces next next position 
...
%     end
% end
% freespace = (open_count / 100) * 2 - 1; 
% freespace = max(-1, min(1, freespace));
% %state_action_feats(5, action) = freespace;  %how much open space is reachable from next_head_loc
