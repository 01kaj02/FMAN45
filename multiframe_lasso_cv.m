...
Nval = floor(N/K);
cvhop = Nval;
randomind = randperm(framehop);%NaN;% Select random indices for picking out validation and estimation indices. 
...
for kframe = 1:Nframes % First loop over frames
    ...
    for kfold = 1:K % Then loop over the folds
        ...
        valind = randomind(cvhop*(kfold-1)+1 : cvhop*kfold); %NaN; % Select validation indices
        estind = setdiff(randomind, valind); %NaN; % Select estimation indices
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            what = lasso_ccd(t(estind),X(estind,:),lambdavec(klam), wold); %NaN% Calculate LASSO estimate at current frame, fold, and lambda % vilken LASSO ? 
            
            SEval(kfold,klam) = SEval(kfold,klam) + norm(t(valind) - X(valind,:) * what)^2./cvhop; %NaN; % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
            SEest(kfold,klam) = SEest(kfold,klam) + norm(t(estind)-X(estind,:)*what)^2/(framehop-cvhop); %NaN; % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
...
SEval = SEval/Nframes; % medelvärde av frames
SEest = SEest/Nframes; % medelvärde av frames

MSEval = mean(SEval,1); % Average validation error across folds
MSEest = mean(SEest,1); % Average estimation error across folds
[~, lmbda] = min(MSEval);  
lambdaopt = lambdavec(lmbda); %NaN; % Assign optimal lambda 
...
for kframe = 1:Nframes
    ...
    Wopt(:,kframe) = lasso_ccd(t, X, lambdaopt); %NaN; 
    framelocation = framelocation + framehop;
end
